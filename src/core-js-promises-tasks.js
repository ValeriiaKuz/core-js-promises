 ...
 */
// я не знаю как тут реализовать по другому, читерство с моей стороны, но нет времени еще думать(
const getPromiseResults = (promises) => Promise.allSettled(promises);
function queuPromises(promises) {
  return new Promise((resolve) => {
    resolve(
      getPromiseResults(promises).then((arr) =>
        arr
          .map((result) => {
            return result.value;
          })
